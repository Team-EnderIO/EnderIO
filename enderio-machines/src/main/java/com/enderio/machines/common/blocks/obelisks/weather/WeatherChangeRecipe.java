...
public record WeatherChangeRecipe(FluidStack fluid, WeatherMode mode) implements MachineRecipe<WeatherChangeRecipe.Input> {
    ...
    public enum WeatherMode implements StringRepresentable{
        ...
        public static final IntFunction<WeatherMode> BY_ID = ByIdMap.continuous(Enum::ordinal, values(), ByIdMap.OutOfBoundsStrategy.ZERO);
        public static final StreamCodec<ByteBuf, WeatherMode> STREAM_CODEC = ByteBufCodecs.idMapper(BY_ID, Enum::ordinal);
    ...
    public static class Serializer implements RecipeSerializer<WeatherChangeRecipe> {
        public static final MapCodec<WeatherChangeRecipe> CODEC = RecordCodecBuilder.mapCodec(inst -> inst.group(
            FluidStack.CODEC.fieldOf("fluid").forGetter(WeatherChangeRecipe::fluid),
                WeatherMode.CODEC.fieldOf("mode").forGetter(WeatherChangeRecipe::mode))
            .apply(inst, WeatherChangeRecipe::new));

        public static final StreamCodec<RegistryFriendlyByteBuf, WeatherChangeRecipe> STREAM_CODEC = StreamCodec.composite(
            FluidStack.STREAM_CODEC,
            WeatherChangeRecipe::fluid,
            WeatherMode.STREAM_CODEC,
            WeatherChangeRecipe::mode,
            WeatherChangeRecipe::new);
