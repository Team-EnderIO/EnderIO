package com.enderio.machines.client.blockentity;
...
public class FluidTankBER implements BlockEntityRenderer<FluidTankBlockEntity> {
    ...
    public void render(FluidTankBlockEntity blockEntity, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight,
        ...
        if (tank.getFluidAmount() > 0) {
            ...
            if (ItemBlockRenderTypes.canRenderInLayer(fluid.getFluid().defaultFluidState(), RenderType.translucent())) {
                renderFluid(fluid, tank.getFluidAmount() / (float) tank.getCapacity(), last.pose(), last.normal(), bufferSource.getBuffer(RenderType.translucent()));
            } else if (ItemBlockRenderTypes.canRenderInLayer(fluid.getFluid().defaultFluidState(), RenderType.solid())) {
                renderFluid(fluid, tank.getFluidAmount() / (float) tank.getCapacity(), last.pose(), last.normal(), bufferSource.getBuffer(RenderType.solid()));
            }
        }
    }
    ...
    private void renderFluid(FluidStack fluidStack, float fillAmount, Matrix4f pose, Matrix3f normal, VertexConsumer consumer) {
        // Get fluid crap
