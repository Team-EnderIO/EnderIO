...
public class RedstoneDoubleChannelFilterMenu extends AbstractContainerMenu {
    ...
    protected RedstoneDoubleChannelFilterMenu(@Nullable MenuType<?> pMenuType, int pContainerId, Inventory inventory, ItemStack pStack) {
        ...
        filter = resourceFilter.map(filter -> {
            if (filter instanceof RedstoneInsertFilter cap) {
                return cap;
            }
        ...
        channels = resourceFilter.map(filter -> {
            if (filter instanceof DoubleRedstoneChannel cap) {
                return cap;
            }
            throw new IllegalArgumentException();
    ...
    public static RedstoneDoubleChannelFilterMenu factory(@Nullable MenuType<RedstoneDoubleChannelFilterMenu> pMenuType, int i, Inventory inventory, FriendlyByteBuf byteBuf) {
    ...
    public void setFirstChannel(ColorControl colorControl) {
        ...
        CoreNetwork.sendToServer(new DoubleChannelPacket(channels.getFirstChannel(), channels.getSecondChannel()));
    }
