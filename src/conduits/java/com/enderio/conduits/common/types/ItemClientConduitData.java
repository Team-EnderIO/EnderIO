...
public class ItemClientConduitData implements IClientConduitData<ItemExtendedData> {
    ...
    public List<AbstractWidget> createWidgets(Screen screen, Supplier<BlockPos> blockPos, Supplier<IConduitType<?>> type, ItemExtendedData extendedConduitData, Supplier<Direction> direction, Vector2i widgetsStart) {
        ...
        widgets.add(new CheckBox(EnderIO.loc("textures/gui/round_robin.png"), widgetsStart.add(110, 20),
            () -> extendedConduitData.get(direction.get()).roundRobin,
            bool -> {
        ...
        widgets.add(new CheckBox(EnderIO.loc("textures/gui/self_feed.png"), widgetsStart.add(130, 20),
            () -> extendedConduitData.get(direction.get()).selfFeed,
            bool -> {
