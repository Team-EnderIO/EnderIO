...
public class ChemicalTicker extends MultiCapabilityAwareConduitTicker<ChemicalConduit, ConduitNetworkContext.Dummy, ChemicalConduitData, IChemicalHandler<?, ?>> {
    ...
    protected void tickCapabilityGraph(
        ...
        for (var extract : extractCaps) {
            tickExtractCapability(conduit, extract.capability(), extract.data(), insertCaps);
        }
    }
    ...
    private <C extends Chemical<C>, S extends ChemicalStack<C>> void tickExtractCapability(ChemicalConduit conduit, IChemicalHandler<C, S> extractHandler,
        ...
        if (!chemicalExtendedData.lockedChemical.isEmpty()) {
            ...
            result = extractHandler.extractChemical((S) chemicalExtendedData.lockedChemical.getChemical().getStack(conduit.transferRate()), Action.SIMULATE);
        } else {
        ...
        for (var insert : insertCaps) {
            ...
            if (!chemicalExtendedData.lockedChemical.isEmpty()) {
                transferredChemical = tryChemicalTransfer(destinationHandler, extractHandler, (S) chemicalExtendedData.lockedChemical.getChemical().getStack(conduit.transferRate() - transferred), true);
            } else {
