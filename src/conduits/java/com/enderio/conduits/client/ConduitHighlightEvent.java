...
public class ConduitHighlightEvent {
    ...
    public static void highlight(RenderHighlightEvent event) {
        if (event.getTarget() instanceof BlockHitResult blockHit) {
            if (Minecraft.getInstance().level.getBlockEntity(blockHit.getBlockPos()) instanceof ConduitBlockEntity conduit) {
                ...
                renderShape(event.getPoseStack(), event.getMultiBufferSource().getBuffer(RenderType.lines()), conduit.getShape().getShapeFromHit(blockHit.getBlockPos(), blockHit), (double)pos.getX() - camPos.x, (double)pos.getY() - camPos.y, (double)pos.getZ() - camPos.z, 0.0F, 0.0F, 0.0F, 0.4F);
            }
        }
    }
    ...
    private static void renderShape(PoseStack poseStack, VertexConsumer consumer, VoxelShape shape, double x, double y, double z, float red, float green, float blue, float alpha) {
        ...
        shape.forAllEdges((p_234280_, p_234281_, p_234282_, p_234283_, p_234284_, p_234285_) -> {
            ...
            f1 /= f3;
            f2 /= f3;
            consumer.vertex(posestack$pose.pose(), (float)(p_234280_ + x), (float)(p_234281_ + y), (float)(p_234282_ + z)).color(red, green, blue, alpha).normal(posestack$pose.normal(), f, f1, f2).endVertex();
            consumer.vertex(posestack$pose.pose(), (float)(p_234283_ + x), (float)(p_234284_ + y), (float)(p_234285_ + z)).color(red, green, blue, alpha).normal(posestack$pose.normal(), f, f1, f2).endVertex();
        });
    }
}
