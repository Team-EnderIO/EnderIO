...
public class ConduitScreen extends EIOScreen<ConduitMenu> {
    ...
    private final List<AbstractButton> typedButtons = new ArrayList<>();
    ...
    protected void renderBg(PoseStack poseStack, float partialTicks, int mouseX, int mouseY) {
        ...
        IConduitScreenData data = menu.getConduitType().getData();
        poseStack.pushPose();
    ...
    private void updateConnectionWidgets(boolean forceUpdate) {
        ...
        for (IConduitType type : menu.getBlockEntity().getBundle().getTypes()) {
    ...
    private void addTypedButton(AbstractButton button) {
        typedButtons.add(button);
        addRenderableWidget(button);
    }

    private <T> void actOnDynamic(Function<DynamicConnectionState, IConnectionState> map) {
    ...
    private void setConnectionState(IConnectionState state) {
        menu.getBlockEntity().getBundle().getConnection(menu.getDirection()).setConnectionState(menu.getConduitType(), menu.getBlockEntity().getBundle(), state);
    }
    private IConnectionState getConnectionState(IConduitType type) {
        return menu.getBlockEntity().getBundle().getConnection(menu.getDirection()).getConnectionState(type, menu.getBlockEntity().getBundle());
    }
