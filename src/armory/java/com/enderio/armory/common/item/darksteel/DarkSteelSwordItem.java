...
public class DarkSteelSwordItem extends SwordItem implements AdvancedTooltipProvider {
    public DarkSteelSwordItem(Properties pProperties) {
        super(ArmoryItems.DARK_STEEL_TIER, pProperties.attributes(createAttributes(ArmoryItems.DARK_STEEL_TIER, 3, -2.4F)));
    }
    ...
    public static Optional<ItemStack> getSkull(LivingEntity pTarget) {
        ...
        if (pTarget.getType() == EntityType.ZOMBIE || pTarget.getType() == EntityType.DROWNED || pTarget.getType() == EntityType.HUSK
            || pTarget.getType() == EntityType.ZOMBIE_VILLAGER) {
