...
public class RangeParticle extends TextureSheetParticle {
    ...
    public RangeParticle(ClientLevel level, Vec3 pos, int range, String colour) {
        super(level, pos.x, pos.y, pos.z);
        this.range = range;
        this.lifetime = 5;
        this.rCol = (float)Integer.parseInt(colour.substring(0,2), 16) / 255;
        this.gCol = (float)Integer.parseInt(colour.substring(2,4), 16) / 255;
        this.bCol = (float)Integer.parseInt(colour.substring(4,6), 16) / 255;
    ...
    public static class Provider implements ParticleProvider<RangeParticleData> {
        ...
        public Particle createParticle(RangeParticleData data, @NotNull ClientLevel level, double x, double y, double z, double xSpeed, double ySpeed,
            ...
            RangeParticle particle = new RangeParticle(level, pos, data.range(), data.colour());
