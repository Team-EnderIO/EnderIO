...
public class MachineBlocks {
    ...
    public static final BlockEntry<FluidTankBlock> FLUID_TANK = REGISTRATE
            ...
            .blockstate((ctx, prov) -> prov.horizontalBlock(ctx.get(), prov.models()
                    ...
                    .end()))
            .item(FluidTankItem.Standard::new)
            .model((ctx, prov) -> {
            })
    ...
    public static final BlockEntry<FluidTankBlock> PRESSURIZED_FLUID_TANK = REGISTRATE
            .block("pressurized_fluid_tank",
                    props -> new FluidTankBlock(props, MachineBlockEntities.PRESSURIZED_FLUID_TANK))
            .properties(props -> props.strength(2.5f, 8))
            .loot(MachinesLootTable::copyNBT)
            .blockstate((ctx, prov) -> prov.horizontalBlock(ctx.get(), prov.models()
                    ...
                    .child("overlay", EIOModel.getExistingParent(prov.models(), EnderIO.loc("block/io_overlay")))
                    .end()))
            .item(FluidTankItem.Enhanced::new)
            .model((ctx, prov) -> {
            })
            .tab(() -> EIOCreativeTabs.MACHINES)
            .build()
    ...
    public static final BlockEntry<ProgressMachineBlock> PRIMITIVE_ALLOY_SMELTER = standardMachine(
            "primitive_alloy_smelter", () -> MachineBlockEntities.PRIMITIVE_ALLOY_SMELTER)
            .blockstate((ctx, prov) -> {
                ModelFile model = prov.models().withExistingParent(ctx.getName(), prov.mcLoc("furnace"))
                        .texture("front", EnderIO.loc("block/primitive_alloy_smelter_front"));
                prov
    ...
    private static BlockBuilder<ProgressMachineBlock, Registrate> soulMachine(String name,
            Supplier<BlockEntityEntry<? extends MachineBlockEntity>> blockEntityEntry) {
        return REGISTRATE
                .block(name, props -> new ProgressMachineBlock(props, blockEntityEntry.get()))
                .properties(props -> props.strength(2.5f, 8))
