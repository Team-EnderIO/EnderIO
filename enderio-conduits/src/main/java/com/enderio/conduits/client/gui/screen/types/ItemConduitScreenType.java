...
public class ItemConduitScreenType extends IOConduitScreenType<ItemConduitConnectionConfig> {
    ...
    public void renderLabels(ConduitMenuDataAccess<ItemConduitConnectionConfig> dataAccess, GuiGraphics guiGraphics,
        ...
        guiGraphics.drawString(font, ConduitLang.CONDUIT_PRIORITY, startX, startY + 56 + 6, 0x000000, false);
        guiGraphics.drawString(font, priority, startX + 68 - font.width(priority), startY + 56 + 6, 0x000000, false);
    ...
    public void createLeftWidgets(ConduitScreenHelper screen, int startX, int startY,
        ...
        var insertSlotPos = dataAccess.conduit().getInventorySlotPosition(ItemConduit.INSERT_FILTER_SLOT);
        screen.addFilterConfigureButton(startX - USABLE_START_X + insertSlotPos.x + 18, startY - USABLE_START_Y + insertSlotPos.y, ItemConduit.INSERT_FILTER_SLOT);

        // Redstone control
        var redstoneChannelWidget = screen.addColorPicker(startX + 16 + 4, startY + 40, ConduitLang.REDSTONE_CHANNEL,
            () -> dataAccess.getConnectionConfig().insertRedstoneChannel(),
            value -> dataAccess.updateConnectionConfig(config -> config.withInsertRedstoneChannel(value)));
        ...
        screen.addPreRenderAction(() -> redstoneChannelWidget.visible = dataAccess.getConnectionConfig()
            .insertRedstoneControl()
            .isRedstoneSensitive());

        screen.addRedstoneControlPicker(startX, startY + 40, EIOLang.REDSTONE_MODE,
            () -> dataAccess.getConnectionConfig().insertRedstoneControl(),
            value -> dataAccess.updateConnectionConfig(config -> config.withInsertRedstoneControl(value)));

        // Priority up/down
        screen.addIconButton(startX + 70, startY + 40 + 16, 9, 9, Component.empty(), ICON_INCREASE, () -> dataAccess
            .updateConnectionConfig(config -> config.withPriority(config.priority() + getIncrement())));
        screen.addIconButton(startX + 70, startY + 40 + 16 + 9, 9, 9, Component.empty(), ICON_DECREASE, () -> dataAccess
            .updateConnectionConfig(config -> config.withPriority(config.priority() - getIncrement())));
    }
