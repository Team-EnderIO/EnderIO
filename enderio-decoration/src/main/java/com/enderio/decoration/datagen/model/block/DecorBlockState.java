...
public class DecorBlockState {
    ...
    public static void lightBlock(DataGenContext<Block, ? extends Block> ctx, RegistrateBlockstateProvider prov) {
        prov.getVariantBuilder(ctx.get()).forAllStates(state -> {
            ...
            ModelFile light = prov.models().withExistingParent(id.getPath(), new ResourceLocation(EIODecor.MODID, "block/lightblock"));
            ModelFile light_powered = prov.models().withExistingParent(id.getPath() + "_powered", new ResourceLocation(EIODecor.MODID, "block/lightblock"));
