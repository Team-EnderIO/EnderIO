...
public class ItemStackEnergy {
    ...
    public static boolean hasEnergy(ItemStack stack, int amount) {
        return stack.getCapability(ForgeCapabilities.ENERGY).map(energyStorage -> energyStorage.getEnergyStored() >= amount).orElse(false);
    }
    ...
    public static int receiveEnergy(ItemStack stack, int maxReceive, boolean simulate) {
        ...
        return energyStorage.map(energy -> energy.receiveEnergy(maxReceive, simulate)).orElse(0);
    }
    ...
    public static int extractEnergy(ItemStack stack, int maxExtract, boolean simulate) {
        var energyStorage = stack.getCapability(ForgeCapabilities.ENERGY);
        return energyStorage.map(energy -> energy.extractEnergy(maxExtract, simulate)).orElse(0);
    }
}
