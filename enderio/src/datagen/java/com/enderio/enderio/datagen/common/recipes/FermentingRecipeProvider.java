...
public class FermentingRecipeProvider extends SubRecipeProvider {
    ...
    public void buildRecipes(HolderLookup.Provider registries, RecipeOutput recipeOutput) {
        ...
        build(new FluidStackTemplate(EIOFluids.ROCKET_FUEL.source().get(), 1000),
        ...
        build(new FluidStackTemplate(EIOFluids.NUTRIENT_DISTILLATION.source().get(), 250),
                sizedFromTag(FluidTags.WATER, 1000), EIOTags.Items.MEAT, EIOTags.Items.CROPS, 200,
                recipeOutput);
        build(new FluidStackTemplate(EIOFluids.FIRE_WATER.source().get(), 1000),
                SizedFluidIngredient.of(EIOFluids.HOOTCH.source().get(), 1000), EIOTags.Items.BLAZE_POWDER,
                Tags.Items.DUSTS_REDSTONE, 400, recipeOutput);
        build(new FluidStackTemplate(EIOFluids.LIQUID_SUNSHINE.source().get(), 1000),
                sizedFromTag(FluidTags.WATER, 1000), EIOTags.Items.NATURAL_LIGHTS,
            EIOTags.Items.SUNFLOWER, 200, recipeOutput);
        build(new FluidStackTemplate(EIOFluids.LIQUID_DARKNESS.source().get(), 1000),
        ...
        build(new FluidStackTemplate(EIOFluids.CLOUD_SEED.source().get(), 1000), sizedFromTag(FluidTags.WATER, 1000),
            EIOTags.Items.PRISMARINE, EIOTags.Items.CLOUD_COLD, 400, recipeOutput);
        build(new FluidStackTemplate(EIOFluids.CLOUD_SEED_CONCENTRATED.source().get(), 500),
    ...
    protected void build(FluidStackTemplate output, SizedFluidIngredient input, TagKey<Item> firstReagent,
            TagKey<Item> secondReagent, int ticks, RecipeOutput recipeOutput) {
        recipeOutput.accept(ResourceKey.create(Registries.RECIPE, EnderIO.id("fermenting/" + output.fluid().getKey().identifier().getPath())),
