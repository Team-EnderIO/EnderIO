...
public class SlicingRecipeProvider extends SubRecipeProvider {
    ...
    public void buildRecipes(HolderLookup.Provider registries, RecipeOutput recipeOutput) {
        ...
        build(EIOItems.ZOMBIE_ELECTRODE.get(),
                ...
                BASE_OPERATION_TIME, recipeOutput);
    ...
    protected void build(Item output, List<Ingredient> inputs, int operationTime, RecipeOutput recipeOutput) {
        recipeOutput.accept(ResourceKey.create(Registries.RECIPE, EnderIO.id("slicing/" + BuiltInRegistries.ITEM.getKey(output).getPath())),
                new SlicingRecipe(new ItemStackTemplate(output), inputs, operationTime), null);
    }
