package com.enderio.enderio.compat.jei.extension;
...
public class ShapedEntityStorageCategoryExtension implements ICraftingCategoryExtension<ShapedEntityStorageRecipe> {
    ...
    public void setRecipe(RecipeHolder<ShapedEntityStorageRecipe> recipeHolder, IRecipeLayoutBuilder builder,
        ...
        ItemStack resultItem = recipe.display().get(0).result().resolve(ContextMap.EMPTY, SlotDisplay.ItemStackContentsFactory.INSTANCE).findFirst().get();
        List<List<ItemStack>> inputs = recipe.placementInfo().ingredients()
            .stream()
            .map(ingredient -> ingredient.getValues()
                .stream().map(h -> new ItemStack(h.value()))
                .toList())
            .toList();
        ...
        if (input.isPresent()) {
            ...
            if (soul != Soul.EMPTY) {
                if (SoulBoundUtils.tryBindSoul(resultItem, soul)) {
                    ...
                    inputs = recipe.placementInfo().ingredients()
                        .stream()
                        .map(ingredient -> ingredient.getValues()
                            .stream().<ItemStack>mapMulti((h, c) -> SoulBoundUtils.getBoundIfCapable(new ItemStack(h.value()), soul)
                                .ifPresent(c))
                            .toList())
                        .toList();
                }
        ...
        if (noData) {
            ...
            inputs = recipe.placementInfo().ingredients()
                ...
                .map(ingredient -> ingredient.getValues().stream()
                    .<ItemStack>mapMulti((i,c) -> {
                        if (SoulBoundUtils.canBindSoul(new ItemStack(i.value()))) {
                            for (var entityType : capturableEntityTypes) {
                                ItemStack item = new ItemStack(i.value());
                                if (SoulBoundUtils.tryBindSoul(item, Soul.of(entityType))) {
                                    c.accept(item);
                                }
                            }
                        } else {
                            c.accept(new ItemStack(i.value()));
                        }
                    })
                    .toList())
                .toList();
    ...
    @Override
    public List<SlotDisplay> getIngredients(RecipeHolder<ShapedEntityStorageRecipe> recipeHolder) {
        return recipeHolder.value().placementInfo().ingredients().stream().map(Ingredient::display).toList();
    }
