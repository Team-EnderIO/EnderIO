...
public class NewCapacitorBankBER implements BlockEntityRenderer<NewCapacitorBankBlockEntity> {
    ...
    public NewCapacitorBankBER(BlockEntityRendererProvider.Context context) {

    }

    @Override
    public void render(NewCapacitorBankBlockEntity blockEntity, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight,
        int packedOverlay) {
    ...
    private static void renderBar(NewCapacitorBankBlockEntity blockEntity, PoseStack poseStack, MultiBufferSource bufferSource, Direction facing) {
    ...
    private static void renderIO(NewCapacitorBankBlockEntity capacitorBank, PoseStack poseStack, MultiBufferSource bufferSource, Direction facing) {
     ...
     */
    private static DisplayMode getDisplayModeRelative(NewCapacitorBankBlockEntity blockEntity, Direction horizontalFacing, Vector2i relative) {
        if (blockEntity.getLevel() == null) {
    ...
    private static DisplayMode getDisplayModeRelative(Level level, Direction horizontalFacing, BlockPos pos, Vector2i relative, CapacityTier tier) {
        ...
        if (level.getBlockEntity(pos) instanceof NewCapacitorBankBlockEntity capacitorBank && capacitorBank.getTier() == tier) {
    ...
    @Override
    public AABB getRenderBoundingBox(NewCapacitorBankBlockEntity blockEntity) {
        var pos = blockEntity.getBlockPos();
        return AABB.ofSize(new Vec3(pos.getX(), pos.getY(), pos.getZ()), 32, 32, 32);
    }

    //TODO test performance, potentially try to cache
    private static Size findSize(NewCapacitorBankBlockEntity capacitorBank, Direction facing) {
        int x0 = 0;
        int y0 = 0;
        int x1 = 0;
        int y1 = 0;
    ...
    private static Size validateSize(Size size, NewCapacitorBankBlockEntity capacitorBank, Direction facing) {
    ...
    private record Size(int x0, int y0, int x1, int y1) {
        ResourceLocation getTexture() {
            ...
            if (isSmallY) {
                if (isLeft) {
                    return EnderIO.rl("block/capacitor_additionals/small_r");
                }

                if (isRight) {
                    return EnderIO.rl("block/capacitor_additionals/small_l");
                }

                return EnderIO.rl("block/capacitor_additionals/small_lr");
            }

            if (isSmallX) {
                if (isUp) {
                    return EnderIO.rl("block/capacitor_additionals/small_u");
                }

                if (isDown) {
                    return EnderIO.rl("block/capacitor_additionals/small_d");
                }

                return EnderIO.rl("block/capacitor_additionals/small_ud");
            }

            if (isUp) {
                if (isLeft) {
                    return EnderIO.rl("block/capacitor_additionals/corner_tr");
                }

                if (isRight) {
                    return EnderIO.rl("block/capacitor_additionals/corner_tl");
                }
                
                return EnderIO.rl("block/capacitor_additionals/side_t");
            }

            if (isDown) {
                if (isLeft) {
                    return EnderIO.rl("block/capacitor_additionals/corner_br");
                }

                if (isRight) {
                    return EnderIO.rl("block/capacitor_additionals/corner_bl");
                }

                return EnderIO.rl("block/capacitor_additionals/side_b");
            }

            if (isLeft) {
                return EnderIO.rl("block/capacitor_additionals/side_r");
            }

            if (isRight) {
                return EnderIO.rl("block/capacitor_additionals/side_l");
            }
