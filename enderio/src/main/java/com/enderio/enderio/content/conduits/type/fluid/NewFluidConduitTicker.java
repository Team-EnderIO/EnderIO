...
public class NewFluidConduitTicker extends ConduitTickerBase<FluidConduit> {

    public static final NewFluidConduitTicker INSTANCE = new NewFluidConduitTicker();

    private NewFluidConduitTicker() {}
    ...
    protected void tickNetwork(ServerLevel level, ConduitNetwork network, int tickOffset) {
        final int tickRate = getTickRate(network);
        ...
        for (var channel : network.allChannels()) {
            for (var extractConnection : network.extractConnections(channel)) {
                ...
                if (!context.lockedFluid().isSame(Fluids.EMPTY)) {
                    doFluidTransfer(context.lockedFluid(), fluidRate, tickRate, extractConnection, insertConnections);
                } else {
                    ...
                    for (int i = 0; i < extractHandler.getTanks() && remaining > 0; i++) {
                        ...
                        remaining = doFluidTransfer(fluid, remaining, tickRate, extractConnection, insertConnections);
    ...
    private int doFluidTransfer(Fluid fluid, int maxTransfer, int tickRate, ConduitBlockConnection extractConnection,
