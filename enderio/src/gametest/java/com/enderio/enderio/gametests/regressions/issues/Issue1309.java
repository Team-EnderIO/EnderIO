    ...
    public static void testIssue1309(final DynamicTest test) {
        ...
        test.onGameTest(ConduitGameTestHelper.class, helper -> {
            ...
            helper.startSequence()
                ...
                .thenExecute(() -> helper.fillAir(1, 1, 1, 3, 1, 1))
                // Place a conduit between the two chests
                .thenExecute(() -> helper.fillConduits(itemConduit, 1, 1, 1, 3, 1, 1))
                // Configure inserts
                .thenExecute(() -> helper
                ...
                .thenExecute(() -> helper
                    .getConduitBundle(2, 1, 1, false)
                    .setConnectionConfig(itemConduit, Direction.SOUTH, ItemConduitConnectionConfig.DEFAULT.withIsExtract(false).withIsInsert(true)))
                .thenExecute(() -> helper
                    .getConduitBundle(3, 1, 1, false)
                    .setConnectionConfig(itemConduit, Direction.SOUTH, ItemConduitConnectionConfig.DEFAULT.withIsExtract(false).withIsInsert(true)))
                // Configure extracts
                .thenExecute(() -> helper
                    .getConduitBundle(1, 1, 1, false)
                ...
                .thenExecute(() ->
                    helper.assertAllNetworksDiffer(itemConduit, 2, 1, 1, 3, 1, 1, false))
                // Ensure first network can transfer items
                .thenExecute(() ->
                    helper.insertIntoContainer(0, 1, 1, Items.DIRT, 1))
                .thenExecuteAfter(tickRate, () -> helper.assertContainerHasExactly(1, 1, 2, Items.DIRT, 1))
                // Ensure second network can transfer items
                .thenExecute(() ->
                    helper.insertIntoContainer(4, 1, 1, Items.DIRT, 1))
                .thenExecuteAfter(tickRate, () -> helper.assertContainerHasExactly(3, 1, 2, Items.DIRT, 1))
                .thenSucceed();
        });
    }
}
