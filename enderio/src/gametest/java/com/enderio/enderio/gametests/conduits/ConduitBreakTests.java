...
public class ConduitBreakTests {
        ...
        test.onGameTest(ConduitGameTestHelper.class, helper -> {
            ...
            helper.startSequence().thenExecute(() -> {
                ...
                if (!breakAsPlayerByConduit(helper, itemConduit)) {
                    throw helper.assertionException("Breaking the last conduit did not remove the bundle.");
                }
        ...
        test.onGameTest(ConduitGameTestHelper.class, helper -> {
            ...
            helper.startSequence().thenExecute(() -> {
                ...
                if (breakAsPlayerByConduit(helper, redstoneConduit)) {
                    throw helper.assertionException("Breaking one of multiple conduits removed the whole bundle.");
                }
        ...
        test.onGameTest(ConduitGameTestHelper.class, helper -> {
            ...
            helper.startSequence().thenExecute(() -> {
                ...
                if (breakAsPlayer(helper)) {
                    throw helper.assertionException("Breaking a facade removed its conduit bundle.");
                }

                var bundle = bundle(helper);
                if (bundle.hasFacade()) {
                    throw helper.assertionException("Breaking the facade did not preserve the conduit.");
                }
                assertOnlyConduitsRemain(helper, itemConduit);
                assertItemFilterRemains(helper, itemConduit);
                ...
                if (!breakAsPlayerByConduit(helper, itemConduit)) {
                    throw helper.assertionException("Breaking the exposed final conduit did not remove the bundle.");
                }
    ...
    public static void breaksFacadeBeforeOneOfMultipleConduits(final DynamicTest test) {
        ...
        test.onGameTest(ConduitGameTestHelper.class, helper -> {
            ...
            helper.startSequence().thenExecute(() -> {
                ...
                var bundle = bundle(helper);
                if (bundle.hasFacade()) {
                    throw helper.assertionException("Breaking the facade did not preserve both conduits.");
                }
                ...
                if (breakAsPlayerByConduit(helper, redstoneConduit)) {
                    throw helper.assertionException("Breaking one conduit removed the whole bundle.");
                }
        ...
        test.onGameTest(ConduitGameTestHelper.class, helper -> {
            ...
            helper.startSequence().thenExecute(() -> {
                ...
                if (!bundle.hasFacade() || !bundle.getConduits().isEmpty()) {
                    throw helper.assertionException("Wrenching the final conduit did not preserve only the facade.");
                }
                assertDrops(helper, ConduitBlockItem.getStackFor(itemConduit, 1), basicItemFilter());
            }).thenSucceed();
        });
        ...
        test.onGameTest(ConduitGameTestHelper.class, helper -> {
            ...
            helper.startSequence().thenExecute(() -> {
                ...
                if (!bundle(helper).hasFacade()) {
                    throw helper.assertionException("Wrenching one conduit removed the facade.");
                }
                assertOnlyConduitsRemain(helper, itemConduit);
                assertItemFilterRemains(helper, itemConduit);
                assertDrops(helper, ConduitBlockItem.getStackFor(redstoneConduit, 1));
    ...
    private static void addConduit(ConduitGameTestHelper helper, Holder<Conduit<?, ?>> conduit) {
        ...
        if (!bundle(helper).hasConduitStrict(conduit)) {
            throw helper.assertionException("Could not add a second conduit to the bundle.");
        }
    ...
    private static void wrenchAsPlayer(ConduitGameTestHelper helper, Holder<Conduit<?, ?>> target) {
        ...
        InteractionResult result = bundle(helper).onWrenched(
        ...
        if (result == InteractionResult.PASS) {
            throw helper.assertionException("Wrenching a conduit was not handled.");
        }
    }
    ...
    private static BlockHitResult createConduitHit(ConduitGameTestHelper helper, BlockPos pos, Holder<Conduit<?, ?>> target) {
        ...
        throw helper.assertionException("Could not create a player hit for conduit " + target.getRegisteredName());
    }
    ...
    private static void assertOnlyConduitsRemain(ConduitGameTestHelper helper, Holder<Conduit<?, ?>>... expectedConduits) {
        ...
        if (bundle.getConduits().size() != expectedConduits.length) {
            throw helper.assertionException("Expected " + expectedConduits.length + " conduits to remain but found " + bundle.getConduits().size());
        }

        for (Holder<Conduit<?, ?>> expectedConduit : expectedConduits) {
            if (!bundle.hasConduitStrict(expectedConduit)) {
                throw helper.assertionException("Expected conduit to remain in the bundle: " + expectedConduit.getRegisteredName());
            }
        }
    }
    ...
    private static void assertItemFilterRemains(ConduitGameTestHelper helper, Holder<Conduit<?, ?>> itemConduit) {
        ...
        if (!ItemStack.isSameItemSameComponents(filter, basicItemFilter())) {
            throw helper.assertionException("The filter on the remaining item conduit was removed.");
        }
    }
    ...
    private static void assertNoBundle(ConduitGameTestHelper helper) {
        if (helper.getConduitBundle(BUNDLE_POS.getX(), BUNDLE_POS.getY(), BUNDLE_POS.getZ(), true) != null) {
            throw helper.assertionException("Expected the conduit bundle to be removed.");
        }
    }
    ...
    private static void assertDrops(ConduitGameTestHelper helper, ItemStack... expectedDrops) {
        ...
        if (actualDrops.size() != expectedDrops.length) {
            throw helper.assertionException("Expected " + expectedDrops.length + " item drops but found " + actualDrops.size());
        }
        ...
        for (ItemStack expected : expectedDrops) {
            ...
            if (!matched) {
                throw helper.assertionException("Expected drop was not found: " + expected);
            }
        }
    }
