    ...
    public static void testSecondItemDoesNotDeleteFirstItem(final DynamicTest test) {
        ...
        test.onGameTest(EnderGameTestHelper.class, helper -> {
            ...
            helper.startSequence()
                ...
                .thenExecute(() -> helper.fillContainer(0, 1, 0, EIOFluids.XP_JUICE.source().get(), 1000))
                
                // Create first damaged golden axe with mending enchantment
                .thenExecute(() -> {
                    ItemStack firstAxe = createDamagedMendingAxe(mendingEnchantment, 20);
                    helper.insertItemIntoSlot(0, 1, 0, FluidTankBlockEntity.FLUID_DRAIN_INPUT.getIndex(), firstAxe);
                })
                
                // Wait for repair to complete (tank ticks every 5 ticks, give it 2 seconds)
                .thenExecuteAfter(40, () -> {
                    helper.assertSlotHasItem(0, 1, 0, FluidTankBlockEntity.FLUID_DRAIN_OUTPUT.getIndex(), Items.GOLDEN_AXE);
                })
                ...
                .thenExecute(() -> {
                    ...
                    helper.insertItemIntoSlot(0, 1, 0, FluidTankBlockEntity.FLUID_DRAIN_INPUT.getIndex(), secondAxe);
                })
                
                // Wait a few more ticks to ensure the tank has processed
                .thenExecuteAfter(20, () -> {
                    ItemStack outputItem = helper.getItemInSlot(0, 1, 0, FluidTankBlockEntity.FLUID_DRAIN_OUTPUT.getIndex());
                    ItemStack inputItem = helper.getItemInSlot(0, 1, 0, FluidTankBlockEntity.FLUID_DRAIN_INPUT.getIndex());
